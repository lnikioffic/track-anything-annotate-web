...
if __name__ == '__main__':
    ...
    if prompts['mode'] == 'point':
        for point_c, point_l in zip(
            prompts['point_coords'], prompts['point_labels']
        ):
            prompt = {
        ...
        for box in prompts['boxes']:
            ...
            masks, scores, logits = seg.predict(
                prompt, prompts['mode'], multimask=True
            )
            masks_list.append(masks[np.argmax(scores)])
    else:
        masks, scores, logits = seg.predict(
            prompts, prompts['mode'], multimask=False
        )
        masks_list = masks
        print(len(masks))
